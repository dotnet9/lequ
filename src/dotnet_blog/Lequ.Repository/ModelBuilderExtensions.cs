...
namespace Lequ.Repository
{
    public static class ModelBuilderExtensions
    {
        ...
        public static void Seed(this ModelBuilder modelBuilder)
        {
            ...
            {
                ID = 1, Name = "C#", Description = "B/S,C/S, App", StatusEnum = ModelStatus.Normal,
                CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
            };
            var categoryCPlusPlus = new Category
            {
                ID = 2, Name = "C++", Description = "C/S, App", StatusEnum = ModelStatus.Normal,
                CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
            };
            modelBuilder.Entity<Category>().HasData(categoryCSharp, categoryCPlusPlus);


            var tagCSharp = new Tag
            {
                ID = 1, Name = "C#", Description = "B/S,C/S, App", StatusEnum = ModelStatus.Normal,
                CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
            };
            var tagCPlusPlus = new Tag
            {
                ID = 2, Name = "C++", Description = "C/S, App", StatusEnum = ModelStatus.Normal,
                CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
            };
            ...
            var albumCSharp = new Album
            {
                ID = 1, Name = "C#", Description = "B/S,C/S, App", StatusEnum = ModelStatus.Normal,
                CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
            };
            var albumCPlusPlus = new Album
            {
                ID = 2, Name = "C++", Description = "C/S, App", StatusEnum = ModelStatus.Normal,
                CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
            };
            ...
            for (var i = 0; i < 30; i++)
            {
                ...
                {
                    BlogID = blog.ID, CategoryID = categoryCSharp.ID, StatusEnum = ModelStatus.Normal,
                    CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
                });
                blogCategories.Add(new BlogCategory
                {
                    BlogID = blog.ID, CategoryID = categoryCPlusPlus.ID, StatusEnum = ModelStatus.Normal,
                    CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
                });

                blogTags.Add(new BlogTag
                {
                    BlogID = blog.ID, TagID = tagCSharp.ID, StatusEnum = ModelStatus.Normal,
                    CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
                });
                blogTags.Add(new BlogTag
                {
                    BlogID = blog.ID, TagID = tagCPlusPlus.ID, StatusEnum = ModelStatus.Normal,
                    CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
                });

                blogAlbums.Add(new BlogAlbum
                {
                    BlogID = blog.ID, AlbumID = albumCSharp.ID, StatusEnum = ModelStatus.Normal,
                    CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
                });
                blogAlbums.Add(new BlogAlbum
                {
                    BlogID = blog.ID, AlbumID = albumCPlusPlus.ID, StatusEnum = ModelStatus.Normal,
                    CreateUserID = lstUsers[random.Next(lstUsers.Count)].ID, CreateDate = DateTime.Now
                });
                ...
                {
                    ID = i + 1, BlogID = blog.ID, Content = "This blog is very good , I like it, thank you!",
                    StatusEnum = ModelStatus.Check, CreateUserID = 1, CreateDate = DateTime.Now
                });
            }
